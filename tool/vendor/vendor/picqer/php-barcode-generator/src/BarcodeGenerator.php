...
use Picqer\Barcode\InvalidArgumentException;
...
abstract class BarcodeGenerator
{
    ...
    protected function getBarcodeData($code, $type)
    {
        switch (strtoupper($type)) {
            ...
            default: {
                throw new \InvalidArgumentException('Char ' . $char . ' is unsupported');
    ...
    protected function encode_code39_ext($code)
    {
        ...
        {
            $chr = [];
            // ... (rest of the $chr array)
            $sum = 0;
            $codelength = strlen($code);
            for ($i = 0; $i < $codelength; ++$i) {
                $k = array_keys($chr, $code[$i]);
                $sum += $k[0];
            }
            $j = ($sum % 43);
    ...
    protected function barcode_c128($code, $type = '')
    {
        ...
        switch (strtoupper($type)) {
            case 'A': // MODE A
                $startid = 103;
                $code_data = []; // Initialize the array
                for ($i = 0; $i < $len; ++$i) {
                    $char = $code[$i]; // Access the character using array-style index
                    $char_id = ord($char);
                    
                    if (($char_id >= 241) && ($char_id <= 244)) {
                        // Handle special characters using $fnc_a
                        if (isset($fnc_a[$char_id])) {
                            $code_data[] = $fnc_a[$char_id];
                        } else {
                            throw new InvalidArgumentException('Char ' . $char . ' is unsupported in FNC mapping');
                        }
                    } elseif (($char_id >= 0) && ($char_id <= 95)) {
                        // Map the character using $keys_a
                        $pos = strpos($keys_a, $char);
                        if ($pos !== false) {
                            $code_data[] = $pos;
                        } else {
                            throw new InvalidArgumentException('Char ' . $char . ' is unsupported in keys mapping');
                        }
                    } else {
                        // Throw exception for unsupported characters
                        throw new InvalidArgumentException('Char ' . $char . ' is unsupported');
                    }
                }
                break;
            ...
            case 'B': // MODE B
                ...
                $code_data = []; // Initialize the array
                for ($i = 0; $i < $len; ++$i) {
                    $char = $code[$i]; // Fixed: Use array-style index
                    $char_id = ord($char);
        
                    if (($char_id >= 241) && ($char_id <= 244)) {
                        // Handle special characters using $fnc_b
                        if (isset($fnc_b[$char_id])) {
                            $code_data[] = $fnc_b[$char_id];
                        } else {
                            throw new InvalidArgumentException('Char ' . $char . ' is unsupported in FNC mapping');
                        }
                    } elseif (($char_id >= 32) && ($char_id <= 127)) {
                        // Map the character using $keys_b
                        $pos = strpos($keys_b, $char);
                        if ($pos !== false) {
                            $code_data[] = $pos;
                        } else {
                            throw new InvalidArgumentException('Char ' . $char . ' is unsupported in keys mapping');
    ...
    protected function barcode_eanupc($code, $len = 13)
    {
        ...
        if ($code_len == $data_len) {
            ...
            throw new Picqer\Barcode\Exceptions\InvalidCheckDigitException("Invalid check digit");
        }
        if ($len == 12) {
            // UPC-A
            $code = '0' . $code;
